% Modified "ukf_update1" in ekfukf toolbox to get the square-root UKF method
...
function [M,P,K,MU,S,LH] = ukf_update1_sr(M,P,Y,h,R,h_param,alpha,beta,kappa,mat)
  ...
  [MU,S,C] = ut_transform_sr(M,P,h,h_param,tr_param,R);
  K = (C / S)/S';
  M = M + K * (Y - MU);
  U = K*S';
  for i=1:size(U,2)
%       P = cholupdate(P,U(:,i),'-');
      [P,p] = cholupdate(P,U(:,i),'-'); % added by Junjian Qi
  end
