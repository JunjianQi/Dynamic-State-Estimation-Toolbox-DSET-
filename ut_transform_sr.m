% Modified "ut_transform" in ekfukf toolbox for the square-root UKF method
...
function [mu,S,C,X,Y,w] = ut_transform_sr(M,P,g,g_param,tr_param,Q)
  ...
  elseif mat
    ...
    X = ut_sigmas_qjj(M,P,c);
    w = {WM,W,c};
  else
    [WM,WC,c] = ut_weights_sr(size(M,1),alpha,beta,kappa);
    X = ut_sigmas_sr(M,P,c);
    w = {WM,WC,c};
  end
  ...
  if mat
    ...
    [SS,S] = qr([sqrt(WC(2))*(Y(:,2:end)-repmat(mu,1,size(Y,2)-1)) chol(Q)]');
    S = S(1:size(Y,1),1:size(Y,1));
    if WC(1)>=0
        S = cholupdate(S,(WC(1))^(1/2)*(Y(:,1)-mu),'+');
    else
        S = cholupdate(S,(-WC(1))^(1/2)*(Y(:,1)-mu),'-');
    end
    for i=1:size(X,2)
